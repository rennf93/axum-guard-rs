---
name: axum-guard-rs
description: Use when working in axum-guard-rs (github.com/rennf93/axum-guard-rs), the axum adapter for the guard-core-rs detection engine: editing this crate, planning or reviewing its middleware implementation, wiring guard-core-rs into axum (tower Layer/Service or axum::middleware), or answering questions about the repository's status. The repo is currently a 14-line cargo new scaffold with zero dependencies (not even axum or tower), no engine integration, and no CI that compiles code; everything here must be described as roadmap, not reality.
---

# axum-guard-rs

Reserved namespace for the axum adapter of the Guard ecosystem. Currently a scaffold: `src/lib.rs` is the stock 14-line `cargo new` stub, `Cargo.toml` has no dependencies, and no CI compiles anything.
...
```bash
git clone https://github.com/rennf93/axum-guard-rs
cd axum-guard-rs
```
...
What does not exist: any axum, tower, or guard-core-rs dependency, any middleware code, any configuration type, any real tests, any CI that compiles the crate. Do not describe this crate as functional, integrated, or published.

## Intended Integration

Roadmap, not reality:

1. Depend on `guard-core-rs` (facade re-exporting `compiler`, `preprocessor`, `semantic`) and `axum` (and/or `tower`).
2. Implement middleware as a `tower::Layer`/`Service` pair or an `axum::middleware::from_fn` wrapper around the router. tower-guard-rs is planned as the generic tower foundation; if it lands first, this crate becomes a thin axum-specific convenience over it.
3. Per request: extract method, path, headers, client IP, and body; call the engine synchronously (it is CPU-bound, no I/O, no tokio, safe to call inside the async handler); return a 403 response on a threat verdict.
...
- Sibling adapters: [tower-guard-rs](https://github.com/rennf93/tower-guard-rs), [actix-guard-rs](https://github.com/rennf93/actix-guard-rs), [rocket-guard-rs](https://github.com/rennf93/rocket-guard-rs).
